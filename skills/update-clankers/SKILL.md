...
2. If running manually, detect installed agents with a single Bash call: `command -v claude copilot cursor-agent opencode codex pi`. Only the ones that resolve are candidates.
...
- GitHub Copilot CLI checks for updates automatically on startup; `copilot update` forces an immediate stable-channel update.
